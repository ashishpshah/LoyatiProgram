	...
	public class MenuController : BaseController<ResponseModel<Menu>>
    {
			...
			try
			{
				if (viewModel != null && viewModel != null)
				{
					...
					try
					{
						...
						CommonViewModel.RedirectURL = Url.Action("Index", "Menu");
		...
		public ActionResult DeleteConfirmed(long Id)
		{
			try
			{
				...
				if (Common.IsSuperAdmin() && !_context.Using<UserMenuAccess>().GetAll().ToList().Any(x => x.MenuId == Id)
					&& _context.Using<Menu>().GetAll().ToList().Any(x => x.Id == Id))
				{
