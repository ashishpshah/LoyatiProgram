...
{
    public class DealerController : BaseController<ResponseModel<Dealer>>
    {
        public DealerController(IRepositoryWrapper repository) : base(repository) { }
        ...
        public IActionResult Index()
        {
            try
            {
                ...
                CommonViewModel.ObjList = new List<Dealer>();

                List<SqlParameter> sqlParameters = new List<SqlParameter>();
                sqlParameters.Add(new SqlParameter("@Id", SqlDbType.BigInt) { Value = 0 });

                dt = DataContext_Command.ExecuteStoredProcedure_DataTable("SP_DEALER_GET", sqlParameters, true);

                if (dt != null && dt.Rows.Count > 0)
                {
                    foreach (DataRow dr in dt.Rows)
                    {
                        CommonViewModel.ObjList.Add(new Dealer()
                        {
        ...
        [HttpPost]
        public JsonResult Save(Dealer viewModel)
        {
            try
            {
                ...
                (IsSuccess, response, Id) = DataContext_Command.ExecuteStoredProcedure("SP_DEALER_SAVE", oParams, true);
        ...
        public ActionResult DeleteConfirmed(long Id = 0)
        {
            ...
            var response = DataContext_Command.ExecuteStoredProcedure("SP_Dealer_Delete", parameters.ToArray());
